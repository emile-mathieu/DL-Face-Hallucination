...
Y-channel rationale:
  - Standard in SR literature (SRCNN, VDSR, EDSR all report Y-channel)
  - Human perception is luminance-dominant; Y-channel isolates that
  - Avoids colour-space artefacts inflating/deflating scores
  - Most likely what the AAAI 2015 paper used (common practice in 2015)
"""
...
HR_SIZE            = (100, 100)  # output HR size
TRAIN_LR_INPUT_SIZE = (48, 48)  # network input size (paper Table 1)
TEST_FIXED_LR_SIZE  = (50, 50)  # test LR size before resizing to network input
TEST_NN_INPUT_SIZE  = (48, 48)  # final network input size at test time
...
# Hyperparameters
# Paper: batch=200, lr=1e-5, momentum=0.9, ~5000 cycles, divide lr by 10 on plateau.
# Deviations noted inline.
BATCH_SIZE    = 32    # paper: 200
NUM_EPOCHS    = 50
LEARNING_RATE = 1e-3  # paper: 1e-5; increased because batch size is smaller
MIN_LR        = 1e-4  # paper: 1e-6
WEIGHT_DECAY  = 0.0   # paper: not mentioned
...
def celeba_crop(pil_img: Image.Image) -> Image.Image:
    """Centre-crop to face region. No-op if CELEBA_CROP is False."""
    if not CELEBA_CROP:
        return pil_img
    w, h = pil_img.size
...
def _to_y(t: torch.Tensor) -> torch.Tensor:
    """CHW or 1CHW RGB [0,1] tensor → 1×1×H×W Y-channel tensor."""
    if t.ndim == 3:
        t = t.unsqueeze(0)
...
def batch_metrics(pred: torch.Tensor, target: torch.Tensor) -> Tuple[float, float]:
    ...
    for b in range(pred.shape[0]):
        tp += compute_psnr(pred[b], target[b])
        ts += compute_ssim(pred[b], target[b])
    return tp / pred.shape[0], ts / pred.shape[0]
...
def save_sample_outputs(model, dataset, device, save_dir: Path):
    ...
    with torch.no_grad():
        ...
        save_image(denormalize_per_image(out[0], mean_b[0].numpy(), std_b[0].numpy()),
