...
def main(mode="bichannel", task="all"):

    run_cnn = task in ("all", "train-cnn")
    run_classical = task in ("all", "classical-only")
    ...
    # 1. Initialize and optionally train/load CNN model
    if run_cnn:
        if mode == "basic":
    ...
    if task == "train-cnn":
        print("CNN training task completed.")
        return
    ...
    totals_container = {}
    if run_cnn:
        totals_container[cnn_label] = {"psnr": 0.0, "ssim": 0.0, "count": 0}
    if run_classical:
        totals_container["sc1"] = {"psnr": 0.0, "ssim": 0.0, "count": 0}
        totals_container["sc2"] = {"psnr": 0.0, "ssim": 0.0, "count": 0}
        totals_container["sfh"] = {"psnr": 0.0, "ssim": 0.0, "count": 0}
...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", nargs="?", choices=["basic", "bichannel"], default="bichannel")
    parser.add_argument("--mode", dest="mode_opt", choices=["basic", "bichannel"], default=None)
    parser.add_argument("--task", choices=["all", "train-cnn", "classical-only"], default="all")
    args = parser.parse_args()

    selected_mode = args.mode_opt if args.mode_opt is not None else args.mode
    main(selected_mode, task=args.task)
