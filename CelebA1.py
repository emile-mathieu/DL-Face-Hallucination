...
def main():
    ...
    if args.command == "train":
        ...
        best_val = math.inf
        epochs_no_improve = 0
        patience = 5  # number of epochs without improvement before lr decay
        start_epoch = 1
        ...
        if args.bichannel_checkpoint and os.path.isfile(args.bichannel_checkpoint):
            ...
            ckpt = torch.load(args.bichannel_checkpoint, map_location=device)
            bichannel.load_state_dict(ckpt["model_state"])
            bichannel.to(device)
        ...
        if args.basic_checkpoint and os.path.isfile(args.basic_checkpoint):
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            basic = BasicCNN()
            ckpt = torch.load(args.basic_checkpoint, map_location=device)
            basic.load_state_dict(ckpt["model_state"])
            basic.to(device)
